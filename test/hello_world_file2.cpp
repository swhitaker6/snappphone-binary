...
png_infop info_ptr;
 ...
 int process_data(png_bytep buffer, png_uint_32 length)
 {
    ...
    printf("calling png_process_data function\n");
...
int initialize_png_reader()
{
  ...
  printf("inside initialize png reader\n");
...
int main() {
  ...
  // printf("calling png malloc\n");
  // png = (png_bytep)png_malloc(png_ptr, filesize);

  // fread(png, 0, filesize, pfile);

  // printf("filesize = %u \n", filesize);

  // printf("size of png = %u \n", (unsigned int) sizeof(png));
